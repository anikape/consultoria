...
const DocumentsPage = () => {
  ...
  const sortClientsByDate = (dateField) => {
    return clients.sort((a, b) => {
      const datesA = a.entreprise.flatMap((e) => e.documents.map((doc) => new Date(doc[dateField])));
      const datesB = b.entreprise.flatMap((e) => e.documents.map((doc) => new Date(doc[dateField])));
  ...
  const filteredClients = sortedClients.filter((client) => {
    ...
    return client.entreprise.some((entreprise) =>
      entreprise.documents.some((doc) =>
        doc.issuanceDate.includes(issuanceDate) || doc.expirationDate.includes(expirationDate)
      )
    );
  });
